...
2. If project-local `.multi-agent-orchestrator/.env` or verified setup state is missing, run `configure` without probe and pass known `--current-provider`, `--current-model`, and `--current-effort` when available. Relay each CLI/auth result, inaccessible reason, discovery source, exhaustive flag, exact model and effort candidates, transport availability, and relaunch command. Ask one decision at a time: enabled providers; primary among them; exact model then exact effort for each enabled provider; transport. Discovery is local-only; never browse for model or effort choices. Explain probe quota risk. Then pass every explicit selection, including `MAO_ENABLED_PROVIDERS`, with `--probe`. Partial selection makes no probe. Disabled providers are not probed. Persist only after all enabled exact model+effort probes and transport validation succeed.
